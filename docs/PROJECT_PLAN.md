...
初始设计基线规划约 28 张核心业务表；P2 可能增加技术表。当前真实状态为 28 张业务表，已经落地 `app_user`、共享基础档案、职业探索、Learning、Resume 和 Application Management；AI 技术表尚未实现。
...
Controller 不机械地与表一一对应，最终会按用例和 API 职责合理合并或拆分。当前仓库中实际有 **23** 个 `@RestController`，课程最低数量指标已达到；后续 Controller 按真实业务需要增加，禁止为了数量机械拆分。
...
| Controller | 18 | 23 | 超过 18 个，按业务职责划分 |
| 前端页面 | 15 | 20 | 20 个主要业务页面 |
| AI 功能 | 3 | 0 | P0 + P1 合计 4 个，P2 继续增加 RAG 等能力 |

数据表、Controller 和前端页面的传统数量指标当前已分别达到 28、23、20，均满足课程最低数量要求；正式落地 AI 功能仍为 0 个，课程最终至少 3 个 AI 功能的要求尚未达到。当前状态以[开发状态](DEVELOPMENT_STATUS.md)为准。
