...
初始设计基线规划约 28 张核心业务表；P2 可能增加技术表。最终字段、主外键、索引和拆分/合并方式仍需详细设计。当前真实状态为 22 张业务表，已经落地 `app_user`、共享基础档案、职业探索、Learning 和 Resume；Application 表及 AI 技术表尚未实现。
...
Controller 不机械地与表一一对应，最终会按用例和 API 职责合理合并或拆分。当前仓库中实际有 **18** 个 `@RestController`，课程最低数量指标已达到；后续 Controller 按真实业务需要增加，禁止为了数量机械拆分。
...
部分能力可以用 Tab、弹窗或详情区域组合，但最终课程统计方式需要与老师要求保持一致。当前 Vue Frontend Foundation 已完成，实际有 **18** 个 routed frontend pages，课程最低数量指标已达到；后续 Application / AI 页面按业务需要增加，不再为了数量硬扩张。
...
|---|---:|---:|---:|
| 数据表 | 18 | 22 | 约 28 张核心业务表，P2 可能增加技术表 |
| Controller | 18 | 18 | 超过 18 个，按业务职责划分 |
| 前端页面 | 15 | 18 | 约 18～20 个主要业务页面 |
| AI 功能 | 3 | 0 | P0 + P1 合计 4 个，P2 继续增加 RAG 等能力 |

数据表、Controller 和前端页面的传统数量指标当前已分别达到 22、18、18，均满足课程最低数量要求；正式落地 AI 功能仍为 0 个，课程最终至少 3 个 AI 功能的要求尚未达到。当前状态以[开发状态](DEVELOPMENT_STATUS.md)为准。
